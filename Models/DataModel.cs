...
namespace MVC_API.Models
{
    public class DataModel
    {
        ...
        public DataTable loadList(string Mode, string sp, string Para1, object[] parameters)
        {
            SqlCommand cmd = null;
            SqlDataReader rdr = default(SqlDataReader);
            DataTable dt = new DataTable("TT");
            ...
            try
            {
                ...
                cn = FunMyCon(ref cn);
                {
                    ...
                    {
                        if (parameters[i] is byte[]) // Handle byte[] (for file chunks, etc.)
                        {
                            cmd.Parameters.Add(new SqlParameter("@Para" + (i + 2).ToString(), SqlDbType.VarBinary)
                            {
                                Value = parameters[i]
                            });
                        }
                        else // Handle other types (assume string for simplicity)
                        {
                            cmd.Parameters.AddWithValue("@Para" + (i + 2).ToString(), parameters[i]);
                        }
                    }
